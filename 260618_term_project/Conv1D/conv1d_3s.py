...
def main():
    ...
    x = layers.Dropout(0.3)(x)  # Dense 레이어 전 강한 드롭아웃
    ...
    model_save_path = os.path.join(os.path.dirname(__file__), "conv1d_3s.keras")
    callbacks = [
    ...
    plt.title("Training and validation MAE (Conv1D 3s)")
    plt.legend()
    
    plot_path = os.path.join(os.path.dirname(__file__), "conv1d_3s_loss.png")
    ...
    for figsize, suffix in plot_configs:
        ...
        plt.suptitle('Prediction vs Actual on Test Episode 1 (Conv1D 3s)', y=1.02)
        plt.tight_layout()
        pred_plot_path = os.path.join(os.path.dirname(__file__), f"conv1d_3s_predictions{suffix}.png")
