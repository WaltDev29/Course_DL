...
def main():
    ...
    model_save_path = os.path.join(os.path.dirname(__file__), "conv1d_120s.keras")
    callbacks = [
    ...
    plt.title("Training and validation MAE (Conv1D 120s)")
    plt.legend()
    
    plot_path = os.path.join(os.path.dirname(__file__), "conv1d_120s_loss.png")
    ...
    for figsize, suffix in plot_configs:
        ...
        plt.suptitle('Prediction vs Actual on Test Episode 1 (Conv1D 120s)', y=1.02)
        plt.tight_layout()
        pred_plot_path = os.path.join(os.path.dirname(__file__), f"conv1d_120s_predictions{suffix}.png")
