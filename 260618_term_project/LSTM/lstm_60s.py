...
def main():
    ...
    sequence_length = 60  # 과거 60초(60 time steps)를 보고 예측
    delay = 600  # 60초 후를 예측
    batch_size = 256
    ...
    model_save_path = os.path.join(os.path.dirname(__file__), "lstm_60s.keras")
    callbacks = [
    ...
    plt.title("Training and validation MAE (LSTM 60s)")
    plt.legend()
    
    plot_path = os.path.join(os.path.dirname(__file__), "lstm_60s_loss.png")
    ...
    for figsize, suffix in plot_configs:
        ...
        plt.tight_layout()
        pred_plot_path = os.path.join(os.path.dirname(__file__), f"lstm_60s_predictions{suffix}.png")
