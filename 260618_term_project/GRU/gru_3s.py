...
def main():
    ...
    # 2. 하이퍼파라미터 설정
    sampling_rate = 1
    sequence_length = 30   # 과거 3초 분량의 데이터를 입력
    delay = 30             # 3초 후를 예측 (원본 인덱스 기준: 100ms * 30 = 3000ms = 3s)
    batch_size = 256
    ...
    plt.title("Training and validation MAE (GRU 3s)")
    plt.legend()
    ...
    for figsize, suffix in plot_configs:
        ...
        plt.tight_layout()
        pred_plot_path = os.path.join(os.path.dirname(__file__), f"gru_3s_predictions{suffix}.png")
