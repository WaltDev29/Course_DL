...
def main():
    ...
    sequence_length = 1500  # 과거 30초(300 time steps)를 보고 예측
    ...
    plot_training_curve(history, title="Training and validation MAE (GRU 30s)", save_path=plot_path)
    
    # 8. 예측 vs 실제 값 비교 시각화 (테스트 세트의 첫 번째 에피소드 전체 사용)
    print("\n[8. 첫 번째 테스트 에피소드 전체(약 300초) 예측 vs 실제 값 시각화 생성 중...]")
    save_prefix = os.path.join(os.path.dirname(__file__), "gru_30s_predictions")
