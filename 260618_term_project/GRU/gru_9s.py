...
def main():
    ...
    # 2. 하이퍼파라미터 설정
    sampling_rate = 1
    sequence_length = 90   # 과거 9초 분량의 데이터를 입력
    delay = 90  # 9초 후를 예측
    batch_size = 256
    ...
    plt.title("Training and validation MAE (GRU 9s)")
    plt.legend()
    ...
    for figsize, suffix in plot_configs:
        ...
        plt.tight_layout()
        pred_plot_path = os.path.join(os.path.dirname(__file__), f"gru_9s_predictions{suffix}.png")
