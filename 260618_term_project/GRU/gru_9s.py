...
def main():
    ...
    # 2. 하이퍼파라미터 설정
    sampling_rate = 1
    sequence_length = 900   # 과거 90초 분량의 데이터를 입력
    prediction_steps = 90
    batch_size = 256
    ...
    model_save_path = os.path.join(os.path.dirname(__file__), "gru_9s.keras")
    callbacks = [
    ...
    # 7. 학습 곡선 시각화 및 저장
    plot_path = os.path.join(os.path.dirname(__file__), "gru_9s_loss.png")
    plot_training_curve(history, title="Training and validation MAE (GRU 9s)", save_path=plot_path)
    
    # 8. 예측 vs 실제 값 비교 시각화 (테스트 세트의 첫 번째 에피소드 전체 사용)
    print("\n[8. 첫 번째 테스트 에피소드 전체(약 300초) 예측 vs 실제 값 시각화 생성 중...]")
    save_prefix = os.path.join(os.path.dirname(__file__), "gru_9s_predictions")
